...
nonnumeric <- model_set %>% dplyr::select(nonnumeric.feature1)%>% 
  st_drop_geometry()
...
```{r}
sel.internal<-model_set %>% dplyr::select(sel.internal.list) %>% st_drop_geometry()
...
```{r}
sel.tract<-model_set %>% dplyr::select(sel.tract.list) %>% st_drop_geometry()
