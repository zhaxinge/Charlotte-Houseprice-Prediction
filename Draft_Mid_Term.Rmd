...
amenities_feature.list<-data %>% dplyr::select(
...
```{r}
sel.amenitie<-data %>% dplyr::select(
  starts_with('college_'),
  starts_with('daycare_'),
  starts_with('MedicalFacilities_'),
  starts_with('library_'),
...
tract_feature.list<-data[tract.list]%>% 
  st_drop_geometry()
...
sel.tract.list<- c( "pctRenterOccupied", "pctWhite","pctPoverty","MedRent" ,"MedHHInc","MedAge","crime_sum", "PctLongCommute", "Pctgrocery",  "test.high","PctBachelor","MedAge",'price')
```
...
```{r}
a = ls()
rm(list=a[which(a!='data' & a !='sel.internal.list' & a !='sel.tract.list' & a !='sel.amenitie')])
```

```{r}

nonnumeric.feature1 <- c("descbuildi", "storyheigh", "aheatingty", "heatedfuel", "extwall", "foundation", "bldggrade","price")

nonnumeric <- data[nonnumeric.feature]%>% 
  st_drop_geometry()
...
```{r}
M2 <- lm(price ~ ., data = data[sel.internal.list])
```

```{r}
M3 <- lm(price ~ ., data = data[sel.tract.list])
```

```{r}
M4 <- lm(price ~., data = sel.amenitie)
```
